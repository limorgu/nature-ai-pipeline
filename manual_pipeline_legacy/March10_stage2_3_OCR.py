...
# --- NEW: Import from your shared brain ---
...
# 1. Configuration (Linked to Config File)
# ---------------------------
SOURCE_BOOKS_ROOT = SOURCE_IMAGES
LIBRARY_ROOT = OCR_LIBRARY
LIBRARY_ROOT.mkdir(parents=True, exist_ok=True)

FINAL_METADATA_PATH = METADATA_LOG
MODEL = CURRENT_MODEL
WORD_THRESHOLD = 200
...
# ---------------------------
# NEW: Process Audit Exporter
# ---------------------------
def export_process_audit(audit_report: List[Dict]):
    """Saves a permanent CSV record of the pipeline progress."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    process_dir = AUDIT_REPORTS / "Process_Reports"
    process_dir.mkdir(parents=True, exist_ok=True)
    
    report_path = process_dir / f"Process_Audit_{timestamp}.csv"
    
    if not audit_report: return

    fieldnames = [
        "book_name", "author", "completion_percentage", 
        "pages_processed", "total_images_in_source", 
        "missing_count", "sequence_gaps"
    ]
    ...
    print(f"📈 Process Audit CSV Exported: {report_path.name}")
...
def run_audit_refresh():
    ...
    # --- NEW: Call the Exporter ---
    export_process_audit(audit_report)
